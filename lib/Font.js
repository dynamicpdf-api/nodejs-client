...
import { FullNameTable } from './FullNameTable.js';
import { FontInformation } from './FontInformation.js'
import fs from "fs";
...
export class Font {
    ...
    static loadRequired = true;

    static init() {
        if (!this.pathToFontsResourceDirectory) {
            const windir = process.env.WINDIR;  // Windows only
            if (windir && windir.length > 0) {
                this.pathToFontsResourceDirectory = path.join(windir, "Fonts");
            }
        }
    }
    ...
    static #getGoogleFontText(name, weight, italic) {
        if (italic != undefined && italic != null && italic == true)
    ...
    /**
        ...
        * @param {boolean || number} bold The value can either be boolen or Font Weight ( number).
        * @param {boolean} italic The boolean value wheather to set font to italic or not.
        * @returns font
        */
    static google(fontName, bold, italic) {
        var font = new Font();
        if (typeof bold == 'boolean' && bold == true)
            font.name = Font.#getGoogleFontText(fontName, 700, italic);
        else if (typeof bold == 'number')
            font.name = Font.#getGoogleFontText(fontName, bold, italic);
        else if (typeof bold == 'boolean' && bold == false)
    ...
    /**
        * Initializes a new instance of the `Font` class using the global font name.
        * @param {string} fontName The global font name.
        * @returns font
        */
    static global(fontName) {
        var font = new Font();
        font.name = fontName;
        return font;
    }
     ...
     * @param {string} fontName The font name of the font file.
     * @param {string} resourceName The resource name for the font.
     * @returns font
     */
    static fromSystem(fontName,resourceName=null){
        ...
        for (const fontDetail of this.fontDetails) {
            if (fontDetail.fontName.toUpperCase() === fontName.toUpperCase()) {
                const fontResource = new FontResource(fontDetail.filePath, resourceName);
    ...
    static loadFonts(){
        this.init();
        if (!this.loadRequired) return;

        if (!this.pathToFontsResourceDirectory || this.pathToFontsResourceDirectory === "") return;

        const files = fs.readdirSync(this.pathToFontsResourceDirectory);

        for (const file of files) {
            const fullPath = path.join(this.pathToFontsResourceDirectory, file);

            if (fs.statSync(fullPath).isDirectory()) continue;

            const buffer = fs.readFileSync(fullPath);

            const nameTable = this.readFontNameTable(buffer);

            if (nameTable !== null && nameTable.fontName) {
                this.fontDetails.push(new FontInformation(nameTable.fontName, fullPath));
            }
        }
        this.loadRequired = false;
    }

    static readFontNameTable(buffer) {
        let nameTable = null;

        try {
            const intTableCount = (buffer[4] << 8) | buffer[5];

            if (intTableCount > 0) {
                
                const bytTableDirectory = buffer.slice(12, 12 + intTableCount * 16);

                for (let i = 0; i < bytTableDirectory.length; i += 16) {
                    const tag = bytTableDirectory.readUInt32BE(i); 
                    // const tag =
                    //     (bytTableDirectory[i] << 24) |
                    //     (bytTableDirectory[i + 1] << 16) |
                    //     (bytTableDirectory[i + 2] << 8) |
                    //     bytTableDirectory[i + 3];

                    if (tag === 0x6E616D65) {
                        nameTable = FullNameTable.fromBuffer(buffer, bytTableDirectory, i);
                        break;
                    }
                }
            }
        } catch (err) {
            console.error("Error reading font name table:", err);
        }
        return nameTable;
    }
